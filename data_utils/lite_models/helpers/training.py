...
def build_dataloader(dataset, cfg_dl, mode: str):
    ...
    if cfg_dl is None:
        ...
        cfg_dl = {
            ...
            "persistent_workers": True if mode == "train" else False,
            "prefetch_factor": 2 if mode == "train" else 1,
        }
    ...
    return DataLoader(
        ...
        shuffle=cfg_dl.get("shuffle_train", True)
        if mode == "train"
        else cfg_dl.get("shuffle_val", False),
        num_workers=cfg_dl.get("num_workers", 4),
        pin_memory=cfg_dl.get("pin_memory", True),
        drop_last=cfg_dl.get("drop_last", mode == "train"),
        persistent_workers=cfg_dl.get("persistent_workers", True)
        if mode == "train"
        else False,  # true only for training
        prefetch_factor=cfg_dl.get("prefetch_factor", 2)
        if mode == "train"
        else 1,  # default 2 only for training
    )
...
def build_single_dataset(
    ...
    if data_type not in ["SEGMENTATION", "DEPTH", "LANE_DETECTION"]:
        raise ValueError(
            f"Unsupported data_type '{data_type}' in build_single_dataset()"
        )
    ...
    if name == "acdc":
        ...
        return CurveLanesDataset(
            dataset_root, aug_cfg=aug_cfg, mode=mode, data_type=data_type
        )
