...
FIXED_HOMOTRANS_DATASETS = ["CULANE", "TUSIMPLE"]

DYNAMIC_HOMOTRANS_DATASETS = ["CURVELANES", "OPENLANE"]
...
class LoadDataEgoLanes:
    def __init__(
        ...
        if not (self.dataset_name in VALID_DATASET_LIST):
            raise ValueError(
                "Unknown dataset! Contact our team so we can work on this."
            )
        ...
        self.masks = sorted(f for f in pathlib.Path(self.mask_dirpath).glob("*.png"))
        ...
        if checkData.getCheck():
            for set_idx, frame_id in enumerate(self.labels):
                ...
                frame_id_from_img_path = (
                    str(self.images[set_idx]).split("/")[-1].replace(".jpg", "")
                )
                if frame_id == frame_id_from_img_path:
                    if (set_idx % 10 == 0) and (self.N_vals < self.val_cap):
                        # Slap it to Val
        ...
        print(
            f"Dataset {self.dataset_name} loaded with {self.N_trains} trains and {self.N_vals} vals."
        )
    ...
    def calcData(self, ego_left, ego_right, ego_path):
        ...
        start_angle = math.atan(
            (ego_path[1][0] * 640 - ego_path[0][0] * 640)
            / abs((ego_path[1][1] * 320 - ego_path[0][1] * 320))
        )

        data = [x_left_lane_offset, x_right_lane_offset, x_ego_path_offset, start_angle]
