...
from data_utils.lite_models.helpers.training import (
    build_single_dataset,
    build_dataloader,
    save_checkpoint,
    get_unique_experiment_dir,
)
from data_utils.lite_models.helpers.depth import (
    denormalize_image,
    center_crop_vit_safe_lower,
    pad_to_target_center,
)
from data_utils.lite_models.helpers.logger import WandBLogger


from model_components.lite_models.DeepLabv3Plus import DeepLabV3Plus
from model_components.lite_models.UnetPlusPlus import UnetPlusPlus
...
class LiteTrainerBase(ABC):
    ...
    def __init__(self, cfg: dict):
        ...
        self.pseudo_labeler_type = self.train_cfg.get(
            "pseudo_labeler_generator", "vitl"
        )  # default vit large (can choose between s, b, l)
        self.pseudo_labeling = (
            False  # will be set to true if enabled in the dataset config
        )
    ...
    def _build_model_stack(self):
        ...
        if network_model == "deeplabv3plus":
            self.model = DeepLabV3Plus(
                ...
                encoder_partial_load=self.backbone_cfg.get(
                ...
                output_channels=self.network_cfg.get(
                    "output_channels", 1
                ),  # for depth estimation, 1 channel depth map
            )

            print(
                f"[SMP] DeepLabV3Plus | encoder={self.backbone_cfg['type']} | "
            ...
            self.model = UnetPlusPlus(
                ...
                encoder_partial_depth=self.backbone_cfg.get("encoder_partial_depth", 4),
                head_activation=self.network_cfg.get(
                    "head_activation", None
                ),  # activation function after each convolutional layer in the regression head (except the last one)
                head_depth=self.network_cfg.get(
                    "head_depth", 1
                ),  # how many convolutional layers to use in the regression head
    ...
    def _build_output_dirs(self):
        ...
        if exp_name == "val":
            print(
                f"Experiment name = {exp_name} : not saving the experiment in the training folder"
            )
            return
    ...
    def _build_datasets(self):
        ...
        for ds_name in self.training_sets:
            ...
            if ds_yaml_key not in dataset_cfg:
                raise ValueError(
                    f"Missing root for dataset '{ds_name}' -> key '{ds_yaml_key}'"
                )
        ...
        if len(train_datasets) > 0:
            ...
            self.train_loader = build_dataloader(
                self.train_dataset, self.dl_cfg, mode="train"
            )
        else:
        ...
        for ds_name in self.validation_sets:
            ...
            if ds_yaml_key not in dataset_cfg:
                raise ValueError(
                    f"Missing path for dataset '{ds_name}' -> key '{ds_yaml_key}'"
                )
            ...
            self.val_loaders[ds_name] = build_dataloader(
                val_dset, self.dl_cfg, mode="val"
            )
    ...
    def _build_logger(self):
        ...
        if enabled:
            self.wb = WandBLogger(
                run_name=self.exp_name, config=self.cfg, log_dir=self.log_dir
            )

    def _build_pseudo_labeler(self):
        """Build the pseudo-labeling model (DepthAnythingV2 large)."""
        from data_utils.lite_models.depth_anything_v2.depth_anything_v2.dpt import (
            DepthAnythingV2,
        )
        ...
        if encoder not in model_configs:
            raise ValueError(
                f"Invalid pseudo-labeler encoder type '{encoder}'. Choose from 'vits', 'vitb', 'vitl', 'vitg'."
            )

        # insert path of depth aything v2
        checkpoint = (
            f"data_utils/lite_models/depth_anything_v2/depth_anything_v2_{encoder}.pth"
        )
        ...
        depth_anything.load_state_dict(
            torch.load(
                checkpoint, map_location="cuda" if torch.cuda.is_available() else "cpu"
            )
        )
    ...
    def _maybe_resume(self):
        ...
        if self.exp_name == "val":
            print(
                "Experiment name is 'val', not resuming optimizer/scheduler/counters even if checkpoint is loaded."
            )
            return
        ...
        # ============================================================
            ...
            print(
                "  [INFO] Fine-tuning mode: resetting optimizer, scheduler and counters"
            )
    ...
    def _build_encoder_decoder(self):
        ...
        if "timm" not in self.backbone_cfg["type"]:
            self.backbone_cfg["type"] = "timm-" + self.backbone_cfg["type"].replace(
                "_", "-"
            )
        return
    ...
    def _generate_pseudo_labels(self, batch):
        ...
        if self.pseudo_labeling:
            ...
            # ------------------------------------------
            batch["gt"] = (
                torch.from_numpy(np.stack(pseudo_depths, axis=0))
                .unsqueeze(1)
                .to(self.device)
            )  # B x 1 x H x W
